...
test("bootstrap prepara un repositorio Git vacío sin copiar un runtime editable", async () => {
  ...
  assert.equal(consumerPackage.devDependencies["create-project-engineering-os"], "0.1.4");
...
test("upgrade es determinista, preserva deuda y admite rollback explícito", { timeout: 120_000 }, async () => {
  ...
  assert.equal(firstPayload.targetVersion, "0.1.4");
  ...
  assert.equal((await readJson(path.join(target, stateRelative))).packageVersion, "0.1.4");
