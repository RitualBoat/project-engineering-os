...
test("bootstrap prepara un repositorio Git vacío sin copiar un runtime editable", async () => {
  ...
  assert.equal(consumerPackage.devDependencies["create-project-engineering-os"], "0.1.3");
...
test("upgrade es determinista, preserva deuda y admite rollback explícito", { timeout: 120_000 }, async () => {
  ...
  assert.equal(firstPayload.targetVersion, "0.1.3");
  ...
  assert.equal((await readJson(path.join(target, stateRelative))).packageVersion, "0.1.3");
  ...
  const baselineState = await readJson(path.join(baselineRoot, stateRelative));
  for (const relative of ["package.json", "package-lock.json"]) {
    assert.equal(stateAfterUpgrade.files[relative].owner, "project");
    assert.equal(
      stateAfterUpgrade.files[relative].source,
      baselineState.files[relative].source,
    );
    assert.equal(
      stateAfterUpgrade.files[relative].sourceHash,
      baselineState.files[relative].sourceHash,
    );
  }
  const syncConverged = await runInstalled(target, "sync", ["--check"]);
  assertSuccessfulConstructor(syncConverged, "sync --check después de upgrade");
  assert.equal(parseJson(syncConverged, "sync tras upgrade").status, "IN_SYNC");
  assert.equal(
    parseJson(syncConverged, "sync tras upgrade").plan.summary.stateUpdate,
    false,
  );
